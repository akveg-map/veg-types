# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Prepare snow exclusion
# Author: Timm Nawrocki
# Last Updated: 2026-08-02
# Usage: Execute in Python 3.9+.
# Description: "Prepare snow exclusion" converts snow exclusion polygons to raster.
# ---------------------------------------------------------------------------
...
snowex_input = os.path.join(input_folder, 'AKVEG_Snow_Exclusion_3338.shp')

# Define intermediate files
snowex_intermediate = os.path.join(intermediate_folder, 'snow_exclusion_int_10m_3338.tif')

# Define output file
snowex_output = os.path.join(output_folder, 'snow_exclusion_10m_3338.tif')

#### CONVERT SNOW EXCLUSION TO RASTER
####____________________________________________________
...
snowex_options = gdal.RasterizeOptions(
    format='GTiff',
...
# Convert the snow exclusion to raster
if not os.path.exists(snowex_intermediate):
    print('Converting snow exclusion to raster...')
    start_time = time.time()
    gdal.Rasterize(snowex_intermediate, snowex_input, options=snowex_options)
...
start_time = time.time()
gdal.Translate(snowex_output, snowex_intermediate, options=cog_options)
end_timing(start_time)
