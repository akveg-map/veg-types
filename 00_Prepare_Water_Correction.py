# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Prepare water correction
# Author: Timm Nawrocki
# Last Updated: 2026-08-02
# Usage: Execute in Python 3.9+.
# Description: "Prepare water correction" converts manual water correction polygons to raster.
# ---------------------------------------------------------------------------
...
water_input = os.path.join(input_folder, 'AKVEG_Water_Override_3338.shp')

# Define intermediate files
water_intermediate = os.path.join(intermediate_folder, 'water_correction_int_10m_3338.tif')

# Define output file
water_output = os.path.join(output_folder, 'water_correction_10m_3338.tif')

#### CONVERT WATER TO RASTER
####____________________________________________________
...
water_options = gdal.RasterizeOptions(
    format='GTiff',
...
# Convert the water to raster
if not os.path.exists(water_intermediate):
    print('Converting water to raster...')
    start_time = time.time()
    gdal.Rasterize(water_intermediate, water_input, options=water_options)
...
start_time = time.time()
gdal.Translate(water_output, water_intermediate, options=cog_options)
end_timing(start_time)
